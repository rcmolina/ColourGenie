...
{
    unsigned int j;
    //unsigned int period = (unsigned int)(bauds_to_samples(1150) / (bit + 1)); // está mal son 1160 o 1161 (1160,52)
	unsigned int period = (unsigned int)(bauds_to_samples(1102) / (bit + 1));   // la velocidad teórica es 1200, pero 1102Hz (cada pulso 453,5 usecs 
																			// y con fs=44100 (22,675usecs), repito 20 pulsos para el nivel bajo y 40 para para el alto
																	// con 1102 tengo un T=907 usecs por ciclo, ->  Nivel corto = 453,5usecs -> con 2 pulsos de fm=4410Hz	 
																	// con 1102 tengo un T=907 usecs por ciclo, ->  Nivel corto = 453,5usecs -> con 4 pulsos de fm=8820Hz	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  

	do {														
		for (j=0; j<period ; j++) {
			//filter_output(level ? p_silence : p_gain , fp);
			fputc( level ? p_silence : 255, fp);	    
		}
		level ^= 1;
	} while (bit--);
}
...
static void print_usage()
{
    ...
    printf( "-f <sampling freq> : 48000, 44100, 22050, 11025 and 8820.\n");	   
